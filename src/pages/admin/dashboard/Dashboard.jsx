    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          setTotalProducts(productsSnapshot.size); // Sets total products
  
          // Fetch Orders
          const ordersSnapshot = await getDocs(collection(fireDB, 'order'));
          setTotalOrders(ordersSnapshot.size); // Sets total orders
  
          // Fetch Users
          const usersSnapshot = await getDocs(collection(fireDB, 'users'));
          setTotalUsers(usersSnapshot.size); // Sets total users
        } catch (error) {
  ...
  return (
    <Layout>
        <section className="text-gray-600  body-font mt-10 mb-10">
            <div className="container px-5 mx-auto mb-10 w-full max-w-screen-xl ">
